...
### ----------------------------------------------------------------------------
### File:    figA5_fit.R
### Purpose: Builds Appendix 5.1 Figures (A5.1.1; A5.1.2 & A5.1.3).
...
pkginstall(c("ggplot2", "dplyr", "tidyr", "coda", "qs"))
...
figA5.1.1 <- ggplot() + 
  ...
  geom_point(data = data_N, aes(x = cohort, y = Q50, shape = stage), color = "red", size = 1.8) +
  geom_ribbon(data = est_N, aes(x = cohort, ymin = Q2.5, ymax = Q97.5), fill = "black", alpha = 0.2) +
  geom_line(data = est_N, aes(x = cohort, y = Q50), color = "black", linewidth = 0.8) +
  geom_point(data = est_N, aes(x = cohort, y = Q50, shape = stage), color = "black", size = 1.8) +
...
ggsave(
  filename = paste0("results/figures/", projects, "/figA5.1.1.pdf"),
  plot = figA5.1.1,
  width = 1961/300,
  height = 1440/300,
  units = "in"
...
ORE2 <- ORE %>%
  mutate(
    ...
    stage = factor(stage2, levels = c("Smolt", "1SW", "2SW"))) %>%
...
figA5.1.2 <- ggplot() + 
  ...
  geom_point(data = data_S, aes(x = cohort, y = Q50, shape = stage), color = "red", size = 1.8) +
  geom_ribbon(data = est_S, aes(x = cohort, ymin = Q2.5, ymax = Q97.5), fill = "black", alpha = 0.2) +
  geom_line(data = est_S, aes(x = cohort, y = Q50), color = "black", linewidth = 0.8) +
  geom_point(data = est_S, aes(x = cohort, y = Q50, shape = stage), color = "black", size = 1.8) +
...
ggsave(
  filename = paste0("results/figures/", projects, "/figA5.1.2.pdf"),
  plot = figA5.1.2,
  width = 1961/300,
  height = 1440/300,
  units = "in"
...
est_L <- data.frame(
  stage2 = rep(c("Smolt", "Surviving
smolt", "Post-smolt", "Maturing
post-smolt", "Non-maturing
post-smolt"), each = 24),
  cohort = as.numeric(rep(1996:2019, times = 5)),
  Q2.5 = as.vector(ORE3[,1]),
  Q25 = as.vector(ORE3[,2]),
  Q50 = as.vector(ORE3[,3]),
  Q75 = as.vector(ORE3[,4]),
  Q97.5 = as.vector(ORE3[,5])
) %>%
...
figA5.1.3 <- ggplot() + 
  ...
  geom_point(data = data_L, aes(x = cohort, y = Q50, shape = stage2), color = "red", size = 1.8) +
  geom_ribbon(data = est_L, aes(x = cohort, ymin = Q2.5, ymax = Q97.5), fill = "black", alpha = 0.2) +
  geom_line(data = est_L, aes(x = cohort, y = Q50), color = "black", linewidth = 0.8) +
  geom_point(data = est_L, aes(x = cohort, y = Q50, shape = stage2), color = "black", size = 1.8) +
...
ggsave(
  filename = paste0("results/figures/", projects, "/figA5.1.3.pdf"),
  plot = figA5.1.3,
  width = 1961/300,
  height = 1440/300,
  units = "in"
