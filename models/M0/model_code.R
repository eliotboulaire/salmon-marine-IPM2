...
### ----------------------------------------------------------------------------
### File:    model_code.R (M0)
### Purpose: NIMBLE model code for the model (M0) of the IPM2.
...
model_code <- nimbleCode({
  ...
  for (s in 1:S) {
    Mu_Alpha1[s] ~ dnorm(mean = 0, sd = 10)
    Sd_Alpha1[s] ~ T(dt(0, 1/(1^2), 1), 0, )
  }
  
  # Survival rate (Theta 1)
  for (c in 1:C) {
    for (s in 1:S) {
      Alpha1[c, s] ~ dnorm(mean = Mu_Alpha1[s], sd = Sd_Alpha1[s])
      
      Logit_Theta1[1:L, c, s] <- Alpha1[c, s] + Beta1 * (L1[1:L, c] - Mean_L1)
      Theta1[1:L, c, s] <- 1/(1+exp(-Logit_Theta1[1:L, c, s]))
    }
  }
  ...
  for (c in 1:C) {
    for (s in 1:S) {
      ...
      N1surv[1:L, c, s] <- N1[1:L, c, s] * Theta1[1:L, c, s]
  ...
  for (s in 1:S) {
    Mu_Alpha2[s] ~ dnorm(mean = 0, sd = 10)
    Sd_Alpha2[s] ~ T(dt(0, 1/(1^2), 1), 0, )
  }
  
  # Maturation rate (Theta 2)
  for (c in 1:C) {
    for (s in 1:S) {
      Alpha2[c, s] ~ dnorm(mean = Mu_Alpha2[s], sd = Sd_Alpha2[s])
      
      Logit_Theta2[1:L, c, s] <- Alpha2[c, s] + Beta2 * (L2[1:L, c] - Mean_L2)
      Theta2[1:L, c, s] <- 1/(1+exp(-Logit_Theta2[1:L, c, s]))
    }
  }
  ...
  for (c in 1:C) {
    for (s in 1:S) {
      ...
      N2m[1:L, c, s] <- N2[1:L, c, s] * Theta2[1:L, c, s]
      N2nm[1:L, c, s] <- N2[1:L, c, s] * (1-Theta2[1:L, c, s])
