...
### ----------------------------------------------------------------------------
### File:    model_code.R (M6)
### Purpose: NIMBLE model code for the model (M6) of the IPM2.
...
model_code <- nimbleCode({
  ...
  for (s in 1:S) {
    Alpha2[s] ~ dnorm(mean = 0, sd = 10)
  }
  
  # Maturation rate (Theta 2)
  for (c in 1:C) {
    for (s in 1:S) {
      Logit_Theta2[1:L, c, s] <- Alpha2[s] + Beta2 * (L2[1:L, c] - Mean_L2)
